...
{           
  ...
  if (distance_L<30) {
    ...
    analogWrite(Motor_L_pwm_pin, HIGH);
  ...
  digitalWrite(Motor_R_dir_pin, HIGH);
  digitalWrite(Motor_R_pwm_pin, LOW);                    //OR right();
  ...
  if (Serial.available() > 0){                                                          //ex3
    String message = Serial.readStringUntil('\n'); 
    Serial.print("Message received, content: ");  
    Serial.println(message);
    int pos_s = message.indexOf("LCD");
    lcd.setCursor(0, 0);
    lcd.clear();
    ...
    if (pos_s > -1){
      ...
      Serial.println("No message found, try typing LCD:text\n");
    }
  }
